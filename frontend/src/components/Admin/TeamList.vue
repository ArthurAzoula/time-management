                        ...
                        <v-row>
                            ...
                            <v-col cols="12">
                                <v-select
                                    ...
                                    :items="managers.map((manager) => manager.email)"
                                    item-text="email"
                                    item-value="id"
                                    label="Manager"
                                ></v-select>
...
const saveTeam = () => {
    if (editedIndex.value > -1) {
        ...
        teamService.createTeam(editedItem.value).then((response) => {
...
const managerSelection = computed({
    get() {
        return editedItem.value.manager ? editedItem.value.manager.email : null
    },
    set(value) {
        if (!editedItem.value.manager) {
            editedItem.value.manager = {}
        }
        editedItem.value.manager.email = value
    },
})
