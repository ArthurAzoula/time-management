<template>
    <div class="flex flex-col md:flex-row h-full">
        <button class="fixed top-4 left-4 z-50 md:hidden" @click="toggleMenu">
        ...
        <div :class="[
            'menu-layout fixed md:top-0 md:left-0 md:bottom-0 md:m-6 md:rounded-lg md:bg-button-300 md:w-1/6 md:h-full flex flex-col items-start p-4 transition-transform duration-300 z-10',
            {
                'top-0 left-0 w-full h-auto bg-button-300': isMenuOpen,
                '-translate-y-full': !isMenuOpen && !isDesktop,
                'translate-y-0': isMenuOpen || isDesktop,
            },
        ]">
            <div class="flex flex-col items-center mb-8 w-full">
                <router-link to="/me" @click="setActive('myProfil')">
                    <img class="rounded-full mb-6 mt-4 w-28 h-28 object-cover"
                        src="https://randomuser.me/api/portraits/men/1.jpg" alt="Profile Picture" />
                ...
                <ul class="flex flex-col space-y-4 ml-4 mr-4">
                    <router-link to="/" class="font-regular">
                        <li class="flex items-center gap-4 w-full hover:bg-menu-100 cursor-pointer transition duration-200 ease-in-out p-2 transform hover:p-2 rounded"
                            @click="setActive('dashboard')" :class="{ active: activeLink === 'dashboard' }">
                            <DashboardIcon />
                            <p>Dashboard</p>
                        </li>
                    </router-link>
                    <router-link to="/workingtimes" class="font-regular">
                        <li class="flex items-center gap-4 w-full hover:bg-menu-100 cursor-pointer transition duration-200 ease-in-out p-2 transform hover:p-2 rounded"
                            @click="setActive('workingtimes')" :class="{ active: activeLink === 'workingtimes' }">
                            <WorkingTimeIcon />
                            <p>Working time</p>
                        </li>
                    </router-link>
                    <router-link to="/statistics" class="font-regular">
                        <li class="flex items-center gap-4 w-full hover:bg-menu-100 cursor-pointer transition duration-200 ease-in-out p-2 transform hover:p-2 rounded"
                            @click="setActive('statistics')" :class="{ active: activeLink === 'statistics' }">
